...
### BUG-20: Afirmaciones de práctica clínica y operativa sin respaldo en la fuente
* **Severidad**: Alta (deontológica y de fidelidad)
* **Componentes / Páginas**: `index.astro`, `colicos-del-lactante-burriana.astro`, `osteopatia-bebes-burriana.astro`, `fisioterapia-infantil-burriana.astro`, `embarazo-posparto.astro`, `fisioterapia-adultos-burriana.astro`, `contacto.astro`, `DemoForm.astro`, `Timeline.astro`, `404.astro` y las dos páginas legales
* **Problema**: pese a las pasadas anteriores, las rutas clínicas seguían afirmando como hechos actuales que Centro Spai explica o acompaña personalmente («Os explicamos qué suele ocurrir, cómo podemos acompañaros»), realiza valoraciones («Valoración suave del bebé», «Observamos y valoramos con técnicas suaves»), aplica técnicas suaves o manuales adaptadas, ayuda a recuperarse («Te ayudamos a cuidar tu cuerpo… y a recuperarte a tu ritmo»), trata al bebé («Tratamos al bebé, acompañamos a la familia») o determina el enfoque de cada persona («En la valoración os explicamos qué enfoque encaja mejor»). En adultos se atribuía titulación al prestador de cada técnica. El formulario prometía uso y respuesta de datos («Solo pedimos lo necesario para poder responderte», «Lo usaremos solo para responder a tu solicitud», «en la web real este formulario conectará con el equipo»). Y las páginas legales se presentaban como política válida pese a contener placeholders y a que el formulario no procesa datos.
* **Impacto**: el mockup atribuía al centro un protocolo clínico, una capacidad operativa y un tratamiento de datos que nadie ha verificado, con el riesgo deontológico y legal que eso supone en un sitio sanitario.
* **Corrección**:
  * Cada afirmación se resolvió por una de las tres vías admitidas: reescritura como contenido publicado por el centro, reescritura como propuesta dentro de un bloque con la etiqueta «Propuesta de experiencia para el rediseño», o eliminación. Se añadió esa etiqueta al enfoque de osteopatía y al de embarazo/posparto, que aún no la tenían.
  * En adultos se eliminó «Cada una la presta un profesional con su propia titulación», manteniendo el inventario histórico y su aviso sanitario.
  * El formulario y su página solo afirman lo verificable: **«Formulario demostrativo: no envía ni almacena datos»**. Los textos de campo pasan a «Campo de ejemplo», el consentimiento a «acepto continuar con esta solicitud demostrativa» y el mensaje final a «Nada de lo que has escrito ha salido de esta página». Teléfonos, email, dirección y WhatsApp siguen visibles como datos del centro.
  * Las dos páginas legales pasan a una presentación neutra: sin placeholders (`[NIF]`, `[Razón social]`, `[Domicilio social]`), sin responsables, plazos, consentimientos ni condiciones inventadas. Explican qué deberá incorporarse antes de una publicación real, informan de que la web no recoge datos y conservan el aviso sanitario general. Las rutas y los enlaces del pie no cambian.
  * Copy editorial menor: fuera «Si ya venís con vuestros hijos…», «Sin compromiso», «a tu ritmo» y la promesa del 404 («Te ayudamos a encontrar lo que buscas»).
  * `verify-build.mjs` pasa a **47 frases prohibidas** y comprueba que las páginas legales no conserven marcadores; `qa-browser-tests.mjs` añade el aviso del formulario y la neutralidad de las páginas legales.
